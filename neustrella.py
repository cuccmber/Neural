...
os.environ[
    'TF_CPP_MIN_LOG_LEVEL'] = '3'  # 0 = all messages are logged, 3 - INFO, WARNING, and ERROR messages are not printed
...
if __name__ == "__main__":
    ...
    seed = "Today"  # Initial symbol of the text
    horo_count = 0  # Insertion index

    parser = argparse.ArgumentParser()
    parser.add_argument("-t", "--text", action="store", required=False, dest="text", help="Input text file")
    parser.add_argument("-e", "--epochs", action="store", required=False, dest="epochs",
                        help="Number of training epochs")
    parser.add_argument("-p", "--phrase_len", action="store", required=False, dest="phrase_len",
                        help="Phrase analyse length")
    parser.add_argument("-o", "--out_len", action="store", required=False, dest="out_len", help="Output text length")
    parser.add_argument("-g", "--generate", action="store_true", required=False, dest='generate',
                        help="Generate output only without training")
    parser.add_argument("-horo", "--horoscope", action="store", required=False, dest='horo_count',
                        help="Insertion index")
    ...
    if args.horo_count is not None:
        horo_count = int(args.horo_count)

    # Load text data
    pristine_input, pristine_output = False, False
    x, y, x_val, y_val, vectorizer = load_data(data_file, use_words, pristine_input, pristine_output, batch_size,
                                               seq_length)
    ...
    res = res.replace("\n", "")
    # res = res[::-1]
    # res = res.replace(res[0:res.find(" ") + 1], ". ")[::-1]
    res += "\n"

    file1 = open("/home/diane/vs/neustrella/index.html", "r")
    html_doc = file1.read()
    file1.close()
    soup = BeautifulSoup(html_doc, "lxml")
    soup.find_all("div", {"class": "column__text"})[horo_count].string.replaceWith(res)
    file2 = open("/home/diane/vs/neustrella/index.html", "w")
